...
class CubbyHole {
    ...
    public synchronized void put(int value) {
        while (index >= data.length) {
            try {
			    /*
			    when the producer reach the production limit,
			    let consumers to purchase
			    in other words,
			    release the lock, allows other method to modify the variable
			    then notify, to lock again
			     */
                wait(); // waits for notify() call from consumer
            } catch (InterruptedException e) {
            }
